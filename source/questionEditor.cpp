...
void QuestionEditor::printDescription()
{
...
Mode QuestionEditor::processMenu()
{
    ...
    while(true)
    {
        ...
        std::cout << "\t4. Delete existing question" << '\n';
        std::cout << "> ";
        ...
        if(choice == "1") return Mode::New;
        else if(choice == "2") return Mode::Append;
        else if(choice == "3") return Mode::Edit;
        else if(choice == "4") return Mode::Delete;
        else
        {
            std::cout << '\n';
...
void QuestionEditor::makeQuestions()
{
    do
    {
        m_QM.makeQuestion();
...
void QuestionEditor::loadQuestions()
{
...
void QuestionEditor::editQuestions()
{
    if(!m_QM.getAllQuestions().empty())
    {
        ...
        if(choice < 1 || choice > m_QM.getAllQuestions().size()) //throw exception if out of range
        {
...
void QuestionEditor::deleteQuestion()
{
    if(!m_QM.getAllQuestions().empty())
    {
        std::cout << " Choose question to delete: " << "\n\n";
        m_QM.printAllQuestions();
        std::cout << "> ";

        int choice{}; //question number
        std::cin >> choice;
...
bool QuestionEditor::saveToFile()
{
